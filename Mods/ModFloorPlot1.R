...
#        renderTrack(fpp, trk, trk$color, trk$org.x, trk$org.z, 0)
  ...
  if (!is.null(fpp$bayes)) {
    ...
    vendf <- adf[adf$class %in% c("AirVent","AirIntake"),]
    ...
    if (dovents){
      drawcircs(vendf$xm, vendf$ym,radius=0.4, lty=1,lwd=lwd, nv = 12, border=clr)
      if (donames){
        text(vendf$xm,vendf$ym+txtyofs, labels=vendf$name,cex=cex, col=clrtxt)
      }
    }
    if (dovavs){
      drawcircs(vavdf$xm, vavdf$ym,radius=0.4, lty=1,lwd=lwd, nv = 4, border=clr)
      if (donames){
  ...
  if (!is.null(fpp$electric)) {
    ...
    ejdf <- edf[edf$class %in% c("ElecJunc","ElecSource"),]
    ...
    if (dojuncs){
      drawcircs(ejdf$xm, ejdf$ym,radius=0.4, lty=1,lwd=lwd, nv = 12, border=clr)
      if (donames){
...
drawcircs <- function(xv, yv,radius=0.4, lty=1,lwd=2, nv = 12, border="black"){
  ntodo <- length(xv)
  if (ntodo<=0) return;
  for (i in 1:ntodo){
    draw.circle(xv[i],yv[i],radius=radius,lty=lty,lwd=lwd,nv=nv,border=border)
  }
}